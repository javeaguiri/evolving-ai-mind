...
| `workflow-schema.json` rejects `step:N` routing format in condition steps | `on_truthy`/`on_falsy` in `workflow-schema.json` use `bareStepKey` pattern (`^[a-zA-Z0-9][a-zA-Z0-9_]*$`), requiring bare keys like `"4"`. But seed workflows (`create_workflow`, `fix_workflow`, `get_entity`, `diagnose_prompt_schema`, `ping_core`) use `"step:4"` format. `executeCondition` in `step-executor.mjs` normalises both — strips `step:` prefix before building the routing action — so runtime works. Fix: either update `workflow-schema.json` to accept `step:N` format in condition fields, or update all seed condition steps to use bare keys. Failing test suite: `workflow-schema.test.mjs` (5 workflows). |
