...
| `add_entity` — parent entity lookup by name (self-referential FK) | When a domain table has a self-referential FK (e.g. `PGD_Deck.parent_deck_id`), specifying a parent by name at insert time ("parent deck: Spanish Vocabulary") is silently ignored. Root cause: `parse_entity_input` emits `parent_name` but the `add_entity` workflow has no step to resolve a name → existing row ID before the insert. Fix: after `parse_entity_input`, add a `serv_query` step that looks up the parent row by name when `children.deck[*].parent_name` is non-null, then a `js_transform` to set `parent_deck_id` before the iterator inserts. Scope is any domain with a self-referential FK — detected via `PGC_EntitySchema.joins` entries where `parent: "self"`. |
