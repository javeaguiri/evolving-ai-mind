...
All in-scope tracks complete. AC1–AC4, AC7 validated. AC5, AC6, AC8, Track L explicitly deferred to Sprint 6.

**Delivered:**
- Novia Phase 1 (`/novia` command, minds-eye.mjs agentic loop) — N0–N4 all ✅
- 8 read tools + 5 gated write tools (propose_workflow_fix, update_data, insert_data, delete_data, propose_schema_fix)
- Turn limit + action limit gates with Continue / Follow-up / Cancel UX
- Factual + diagnostic memory writes; session thread continuity
- Track S — 11 system context entries; 9 prompts updated; prompt rules framework
- W1 (PGC_WorkflowRunStep fix) + W2 (design_table type rule) + W3 (domain propagation audit B6) + W4 (session_id column) — all ✅
- pgvector active: `search_domain_help` + Pass 2 intent classifier use `PGC_DomainHelp.embedding`
- Modal cancel bug fixed: buttons survive modal cancel
- 358 unit tests pass
...
**2026-06-17 (session 10):** Three fixes deployed. (1) `markdownToBlocks` rewrite in `callback.mjs` (commit 58d3e21): now code-block-aware — splits on fenced code blocks first (capturing regex), emits each as a dedicated single Slack block, applies `\n\n` paragraph splitting only to prose segments. Fixes broken code block rendering where blank lines inside JS functions caused ``` pairs to split across independent Slack blocks. (2) Session ID footer: confirmed `_Session N_` shows `PGC_Session.id` (correct), not `PGC_SessionEntry.id`. Incrementing IDs across `/novia` commands is expected — each fresh command creates one session. (3) Continue with Novia session key mismatch (commit 0b919f5 — **critical functional bug**): `handleMindsEyeViewSubmission` in `interactive.mjs` was sending `existingSessionId` as the SQS key but `handle()` in `minds-eye.mjs` reads `body.sessionId`. Every Continue with Novia click created a fresh session with empty history — Novia had no memory of previous turns. All multi-turn Novia conversations were effectively stateless since the feature launched. Fixed by changing key to `sessionId`. This also explains the `interval_days` design gap: Novia flagged the missing schema column in one turn, but the next "Continue" turn started fresh with no context of that warning, so she implemented with `|| 0` fallback. Pending: `interval_days` schema fix + step 13 workflow update (user to direct Novia tomorrow).
...
**2026-06-18 (session 11):** Three fixes + W3 audit. (1) Batch insert_data: `executeWriteTool` extended to accept `rows` array — full batch sent as single HTTP call to SERV, chunked at 100 rows, counts as one action. Import `insertRows` added to minds-eye.mjs. System prompt catalog updated. (2) Action limit gate: replaces dead-end HUMAN_NOTIFICATION with same UX as turn limit — Continue (resets action count) / Follow-up / Cancel. `postTurnLimitGate` extended with `resetActionCount` flag; callback.mjs gate text differentiates turn vs action limit; interactive.mjs + handleGateResume thread the flag through. (3) `search_domain_help` threshold lowered from 0.5 → 0.4 to match classify-intent; `personal_budget` embedding backfilled. (4) W3 domain propagation audit: B1–B5 all fixed and tested; found untested B6 boundary (`deriveCallScope` in llm-harness.mjs); exported function and added 6 B6 unit tests. (5) Modal cancel bug: clicking Cancel in "Ask Novia" modal stripped buttons from original message permanently — fixed by removing premature `chat.update` from `handleMindsEyeFollowupButton`; buttons now survive modal cancel. Validated. Sprint 6 items: password/key rotation, Track P, AC6, reference table entity separation (create_domain), PGC_DomainHelp embedding on insert.
