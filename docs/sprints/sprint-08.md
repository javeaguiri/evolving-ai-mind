...
**Status: CLOSED 2026-07-25** — opened 2026-07-13.

---

## RETRO — read this before scoping Sprint 9

**The thesis is proven, not asserted.** The `create_workflow` development loop is free: runs 720+721 = 16 `llm_call`s, **0 live**, measured in `PGC_Session`; one paid build ≈ $1.42 (run 729), then every iteration replays for $0. The sprint delivered the one item that removed the per-*success* multiplier (~10 live runs to land one workflow), which always dominated the per-*run* cuts (AC6/AC7, both dropped once iteration became free).

### 1. The harness's first user was the harness

Sprint 7's retro said *"the cost problem is the test loop, not the architecture"* and *"11 of ~15 defects were deterministic."* Sprint 8 built the loop — and the loop immediately found **five deterministic defects in itself and its own design**: an unordered `PGC_StepType` read that reshuffled the injected array (and serialised `updated_at`, so a *touched* row read as a *changed* request), `diffComponents` iterating the raw key union (an 8th phantom component waiting to happen), and the three wedges A10/A11/A12 — none needing an LLM to find, none visible without something comparing two runs byte-for-byte. The tool built to make iteration cheap was the thing that made these findable.

### 2. The signature failure: a rule true of its specimen, promoted to an unconditional

It recurred four times, and it belongs at the top of the next retro's watch-list:

| claim | true of | promoted to | consequence |
|---|---|---|---|
| critic Test 1 — "always removable" | run 700 (options diverge immediately) | all button-interpretation steps | proposed a budget editor that never saves |
| arch-replay §9 — "resolution over HTTP" | `supplied` (the only payload) | all four resolutions | a break with no user-reachable exit (A10/A11) |
| break notice — "keeps the suffix free" | step-local drift | all drift | false for a key several steps read (A12) |
| "no new steps at translation" | steps that break *routing* | *all* steps | rejected functionally-correct output (run 734/735 → Sprint 9) |

Each was correct about its origin, wrong where its assumptions did not hold, and **failed silently** — a workflow that never saves, a notice you cannot act on, a "free" resolution that defers its cost, a working workflow rejected. The discipline that prevents it already exists in the codebase (critic Test 2 states its exception and has never false-positived). **Carry-forward: when a rule is derived from one specimen, write down what about that specimen made it true.**

### 3. When an item keeps changing its justification, the item is the problem

A7 (fingerprint backfill / assembled hash) was wrong **four times in one day** — impossible → a convenience → a correctness mechanism → unnecessary — each position re-derived from whatever evidence had just landed, none asking *what problem is this for*. The user's question dissolved it. Struck, not deferred.

### 4. Reference by content, never copy (rule 2e) — nearly violated inside the sprint that keeps citing it

The design is built on it (recordings are referenced by fingerprint, never copied into the replaying run). Yet the per-key `input` diff was first written into `getReplay` (a second derivation of what the seam already held), and the drift report nearly became another. Both were pulled back to compute-once-at-the-break and read off the frame. The rule is easy to state and easy to re-break.

### 5. The LLM was usually right; the prompt/harness was usually wrong — again

`research_workflow_domain` recommended `upsert_on_conflict` — an implementation directive it had no capability data to ground (it is not given `step_type_contracts`); fixed by scoping the right brain out of implementation, at the fault source, not by defending downstream at the gaps step. The translator produced a functionally-correct workflow (run 734) rejected only for step-count drift. Both point the same way: read what the model was actually given before concluding it misbehaved.

### 6. What the harness could not save — the seed of Sprint 9

`edit_budget` needed one live build against today's prompts (replay cannot rescue a design that never ran). And its generated *runtime* surfaced `create_workflow` **design-quality** defects the harness cannot prevent — a row `limit` that silently dropped the current month, and a period parse ordered after the step that queries on it (the translator improvising `{{selected_period.0}}` around the gap). **AC10 is partial:** the build validated (registered, id 356); the generated workflow's runtime carries to Sprint 9, whose theme is exactly this — create_workflow design/translation quality (bounded drift + query-limit discipline + derive-before-consume), plus the release-readiness work the cost stop preempted.

**Unfinished exercise (not defects):** `soft_drift` never fired (memory did not move between corpora) and `dev_scripts/replay.mjs`'s own file-loop was never driven end-to-end (AC3 was closed directly against the endpoints it wraps). Both carry as exercise, not code.
...
- **AC9 — D3: `notify` template audit.** ✅ **SATISFIED 2026-07-23** — `edit_budget`'s generated `notify`
  (step 18) is clean markdown, not a raw JSON dump; no `design_workflow_dialogs` fix warranted (see C1). Original
  scope note below. **Re-scoped 2026-07-23.** Workflows generated by `create_workflow`
...
| C1 — `notify` template audit (D3 carry-forward) | AC9 | ✅ **DONE 2026-07-23** — audited the `notify` step (18) of the `edit_budget` workflow generated by run 729: **clean markdown** (`Budget updated for {{month}}/{{year}}` + `.length` counts), not a raw JSON dump. No `design_workflow_dialogs` fix warranted — the concern isn't present on this specimen. (`generate_crud_workflows` half dropped — obviated, → backlog.) |
| C2 — `edit_budget` end-to-end validation | AC10 | 🔨 **run 731 diagnosed, 4 fixes landed + deployed 2026-07-24** — the run persisted correctly (Medical → $130, `PGD_Budgets` row 16) but presented wrongly, three separate defects: (1) **Execution** — form-gate resume crashed on a templated `fields` (`resolveFormFields`, a632e6b). (2) **Execution** — standard `**bold**` reached a Slack `mrkdwn` block and rendered literally (`toSlackMrkdwn`, af1e8dc). (3) **Instruction ×3** — `design_workflow_dialogs` v17 (tabular reveals must be GFM pipe tables), `design_workflow_process` v24 (a save-and-continue loop must re-read what it wrote), `workflow_routing_rules` v3 rule 6b (the back-edge lands on the re-query step, not the build step), and **`markdown_formatting_syntax` v6 — the root cause**: its reveal clause claimed tables do not render and told the designer to hand-write `*bold*`, contradicting the rule in (3). Both claims were stale against `callback.mjs`. Re-generate `edit_budget`, then re-run from Slack. **Follow-on 2026-07-24 — experience/procedure partition swept clean:** the v6 fix was itself written with a leak (*"never Slack's single-character forms"*) — a /proc instruction naming the renderer. Rather than reword, every standard-markdown gap is now closed *in code* (`toSlackMrkdwn` also converts headings, task lists and `[text](url)` → `<url|text>`; applied at two label sites that never had it) and the instructions that taught /proc this renderer's syntax are **deleted**: `markdown_formatting_syntax` v10, `design_workflow_dialogs` v18, `design_workflow_process` v25, `review_workflow_redundancy` v3 (the 50-block limit restated as a ~40-field budget). A sweep of all 24 prompts + 36 context rows now reports **zero** references to Slack/mrkdwn/Block Kit. Done in one pass deliberately — incremental prompt edits churn replay fingerprints |
...
### Session 11 — 2026-07-23 — C2 paid run, C1 done, scope pruned, an override bug fixed

**The one paid `edit_budget` build ran clean (run 729) — $1.42, and the sprint's thesis is now demonstrated
end to end.** 9 live `llm_call`s, `completed`, `edit_budget` registered (id 355). Cost pulled **from the proc
logs** — `llm-client` logs `usage.cost.total_cost` (USD) per call; summed by matching the run's `PGC_Session`
trace set (a run spans several execution traceIds, one per gate-resume — the DB `run.trace_id` is *not* the
execution traceId). Per-step: research $0.068, gaps $0.155, design(21) $0.221, redundancy(21r) $0.049,
design(21t) $0.235, dialogs(22) $0.083, steps(23) $0.319, + 2 post-gate = **$1.42**. **21+21t = $0.46** (the
consolidation double-run B1 would have deleted — moot now, since the run fingerprinted itself and every future
`edit_budget` iteration replays for $0). One paid run, then free — proven, not asserted.

**C1 (AC9) DONE — no fix needed.** `edit_budget`'s only `notify` (step 18) is clean markdown
(`Budget updated for {{month}}/{{year}}` + `.length` counts), not a raw JSON dump. The concern D3 guarded
against isn't present on this specimen, so no `design_workflow_dialogs` change is warranted. (`generate_crud_workflows`
half dropped — obviated: 0 workflow steps, no `src` caller, orphaned v9 prompt → backlog cleanup.)

**Scope pruned (all this session, user-directed):** B1/AC6 **dropped** (critic idea abandoned — cost dissolved
by replay, correctness contained by the step-24 gate); D1/AC11 (`/chat` removal) **→ backlog** (deletion
undecided); AC9/C1 re-scoped to `design_workflow_dialogs` only. **Sprint 8's remaining live work is now just
AC10's second half** (run `edit_budget` from Slack) + close.

**An override bug, found by inspecting the intent-map after the run.** The user submitted the step-35a
invocation-phrases gate empty. Its message says *"Enter your own phrases … to **override**"* — but step 35b
added the suggested `intent_keywords` **unconditionally**, so typed phrases *appended* rather than overrode.
Verified: run 729's empty submit wrote 5 `PGC_IntentMap` rows (4 `auto` suggestions + 1 `name`), and had the
user typed phrases they'd have gotten those **plus** all four suggestions. **Fault domain: Instruction** — the
gate's own wording promised a behavior the transform didn't implement, in `create_workflow`'s seed. **Fix A
(true override):** 35b now uses the auto-sources only when the box is empty; typed phrases replace them; the
workflow name is always added. Verified both branches; the existing "override" wording is now accurate.
`create_workflow` **v84 → v85** upserted (seed-only, no code/deploy). This is exactly the kind of deterministic
create_workflow defect the sprint said the free loop exists to catch — found for the price of one DB read.
