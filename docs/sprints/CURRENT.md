...
Separately diagnosed run 624's *actual* recorded failure (AJV errors on `/steps/14` and `/steps/19`, both missing `execution_mode` on `allOf`'s iterator block) — pulled `process_spec.process_design` and `routing_skeleton` from `local_state`: both are clean, 16 items, zero iterator entries, step 13 (`upsert_budget`) correctly designed as `serv_upsert`. The two invalid iterator steps exist only in `generate_workflow_steps`'s own output — it invented steps beyond the locked design (violates "Translation only — no design decisions"), likely misapplying Rule 7 (human_gate button iteration, a field-level `iterator` on an option) as a standalone iterator step for one or both human_gate steps (`confirm_new_categories`, `preview_budget`). **Generation fault, single occurrence** — not fixed; needs a retry to see if it recurs before considering a prompt change. `output_snapshot` for the failed step was `null` (AJV failures abort before persisting output), so the actual invented steps couldn't be inspected directly.

**Next:** user to retry `import_budget_data` creation from Slack now that the serv_upsert schema gaps are deployed. If the iterator/execution_mode Generation fault recurs, treat as an Instruction gap in `generate_workflow_steps` (strengthen the "no design decisions" constraint or clarify Rule 7 iterator-step vs iterator-option distinction).
