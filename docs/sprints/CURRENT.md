...
- Pure seed change — no new step types, no `step-executor.mjs`/`run-workflow.mjs` changes. `node --test tests/unit/*.test.mjs` (378/378 pass, both passes) + `upsert-workflow.mjs create_workflow` (v74→v76 across two passes, pre-write simulation guard passed each time) + `upsert-prompt.mjs` for both prompts. Not yet validated live from Slack.
