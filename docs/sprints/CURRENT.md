...
**2026-06-28 (session 22):** Five fixes to unblock budget bulk insert. (1) `parse_entity_input` v12: "Multiple independent records" rule — when input has N rows of same entity type, return flat array `[{...},...]`; single `{root,children}` form for exactly one record. (2) `select_entity_schema` prompt (new, Haiku, v1, 256 tokens): picks entity_name from user input when multiple entity schemas exist in a domain — handles semantic variants ("budgetary" → "Budget"). (3) `create_domain` step 18 rewritten: now generates one `PGC_EntitySchema` row per root table (tables with no FK pointing to another table in the same domain) — previously hardcoded one schema per domain, silently dropping co-equal root tables like `PGD_Budgets` alongside `PGD_Expenses`. (4) `add_entity` v20: entity resolution chain (steps 1/1a–1e) — serv_query PGC_EntitySchema with limit:50, js_transform normalising flat array vs `{rows}`, condition branch, Haiku LLM selector, js_transform returning plain string, serv_entity_schema; bulk path (steps 2g–2k) — Array.isArray condition, preview build, review gate, serv_insert with full array, notify. (5) `serv_insert` step-executor extended to accept `input.row` as array → single `insertRows` call (no iterator needed). (6) `parse_entity_input` v13: output_schema changed from `type:object` to `oneOf[{object with root},{array}]` — AJV was rejecting the flat array the v12 prompt correctly returned. (7) `chk_budgets_amount` constraint dropped from `PGD_Budgets` — was `> 0`, blocking legitimate negative deduction amounts (taxes, 401k) from budget planner spreadsheet. 366/366 unit tests pass. Deployed. Ready for UC-E1 bulk retest from Slack.

**2026-06-28 (session 21):** `parse_entity_input` v10→v11 — scraped/photographed content rule rewritten. v10 (end of session 20, captured here): initial rule for OCR/receipt/scrape detection; extraction from schema as template; currency cleaning and date normalisation. v11 (session 21): rule fully revised for real-world receipt layout — (a) European number format (comma-decimal); (b) spatial separation: items and prices in separate blocks, stripped of summary lines (TOTAL, BASE, IVA, FECHA, etc.), correlated by sequential position not same-line; (c) quantity-price disambiguation via arithmetic (A × Q ≈ B → A=unit, B=line total); (d) translation: non-English descriptive strings translated to English; schema columns designed to hold foreign-language content (word, phrase, front_text, back_text, native) preserve original language; proper nouns never translated. No domain-specific jargon in rule. Upserted to DB.
