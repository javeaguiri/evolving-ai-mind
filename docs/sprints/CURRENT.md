...
- **Verified against real Novia output (v3→v4):** checked every `respond` message across sessions 928 and 932 (live budget/expense sessions) for how Novia actually renders section headings. She never once uses `#`/`##` — confirms the caution rather than contradicting it. Her proven pattern: **bold text** as the heading label, paired with a `---` horizontal-rule divider between sections. The horizontal rule wasn't in the list at all — added it as a confirmed, evidence-backed convention. Content-only change, no redeploy needed. Not yet re-validated live.
