...
- **Separately verified (this was the deeper ask — guarding against runaway/silent cost explosion) that no endless-looping vector exists, independent of the change above:**
  ...
  - **One related but distinct minor finding, not fixed here** (flagged for a future look): `turnCost` increments by `0.5` on a tool error vs `1.0` on success (`runReasoningLoop`, every tool-type branch) — meaning a persistently-*failing* agent (e.g. hallucinating a malformed tool call repeatedly) gets roughly double the intended number of LLM calls per invocation before `turn_limit` stops it, not fewer. Still hard-bounded (not infinite), but backwards from what you'd want for cost containment — the failure case should probably cost *at least* as much turn budget as success, not half.
