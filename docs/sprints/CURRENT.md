...
**Next:** user to retry `import_budget_data` creation from Slack — the serv_upsert schema gaps are deployed, so `generate_workflow_steps` should now pass on attempt 1 without falling back to the workaround pattern.
