...
**Not yet driven on a live break.** A10's filter and A11's button logic are unit-tested; the Slack render +
click path (I/O, mirroring the `minds_eye_action_gate` pattern) is confirmed at the next live break — which
is free. **Track A remaining:** A8 (dev loop), AC3 (record mode, still never exercised), the A6 `local_state`
diff, `soft_drift` (still never exercised).
