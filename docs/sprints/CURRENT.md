...
**2026-06-09 (session 19):** AC3 follow-up. `expire_prior` confirmed working — old schema snapshot rows (ids 26, 46, 49, 53) correctly expired on new `create_domain` run (443). Run 443 still omitted `card_count` from `initial_value_conventions` despite prompt being current: LLM classifies `card_count DEFAULT 0` as a SQL default and skips case (2). Root cause: case (2) rule described the format but gave no signal that a SQL DEFAULT column can still qualify as an aggregation. Fixed by rewriting case (2): "Any aggregations needed during data loads (even if a SQL DEFAULT is present) — state which fields need to be set with aggregated values..." `create_domain` v17, `design_table` v6. Also found that `upsert-prompt.mjs` and `upsert-system-context.mjs` never bumped DB version on update (`upsert-prompt` omitted `version` from the updates object entirely); `ok` output showed stale DB version (e.g. `v4 ok`) even when seed was at v17, causing repeated false "prompts not deployed" diagnoses. Fixed: `ok` syncs DB version to seed version on mismatch; update path writes `version: seedVersion`; output shows transition (`v4→v17`). All 19 prompt DB versions now synced. `memory-client.mjs` jsonb_contains optimization added to backlog (Medium Priority) with file-path annotation for surfacing at next touch.
