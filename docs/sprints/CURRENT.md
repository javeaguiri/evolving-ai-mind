...
- **New finding, not yet acted on:** Slack's docs also state *"the cumulative limit for all `markdown` blocks in a single payload is 12,000 characters"* — `markdownToBlocks()`'s chunking only enforces `BLOCK_CHAR_LIMIT = 2800` per individual block, with no cap on total blocks/characters across a whole message. A long Novia reply or report could exceed the cumulative limit across multiple blocks and have the whole payload rejected by Slack's API. Not fixed here — flagged for a follow-up decision (add to backlog, or address directly in `callback.mjs`).
