...
**Status: CLOSED 2026-08-06** — opened 2026-07-29. Branch `sprint/09-novia-builds-workflows`.
**Six of nine ACs met.** AC5 half met, AC6 not met, AC8 deferred to Sprint 10.

---

## RETRO — read this before scoping Sprint 10

**The build came free. The runtime did not, and that is the finding.** Novia designed, simulated
and registered `edit_budget` from a Slack conversation with no `create_workflow` involvement at
any point — and she fetched the convention bridge *unprompted, as her first action*, then queried
`PGC_StepType` before designing. Conventions alone were enough to produce a structurally valid,
registered workflow. They were not enough to produce a working one: the runtime needed repairs,
and those were made outside the Novia path. Counting AC5 as met would have been counting the
build and ignoring the AC's second clause.

### 1. A correct registry read looks like invention when the registry is incomplete

The sprint's sharpest recurring finding, three separate instances:

| registry | what it misstated | how it surfaced |
|---|---|---|
| `write_memory` `input_contract` | eight fields declared at the step root that `buildMemoryRow` reads from `step.input` | the moment L0 was pointed at the existing seeds |
| `human_gate` `input_contract` | never documented `fields[].default`, which `step-executor.mjs:490` has always carried through | Novia reached for pre-filling twice and invented a mechanism both times |
| four `create_workflow` prose rows | five claims wrong against the code — most consequentially that an unresolved `{{token}}` "resolves to an empty string" when `resolveTemplate` returns the literal token text | writing the bridge from the engine source instead of from the prose |

The instinct on seeing an agent invent a mechanism is to constrain the agent. In all three cases
the agent was reading correctly and the registry was wrong or silent. **Carry-forward: when
generated output reaches for something that does not exist, check whether the registry declares
it before concluding the generator misbehaved.** This is Sprint 8 retro §5 — *the LLM was usually
right; the prompt/harness was usually wrong* — recurring for a third sprint, now against
registries rather than prompts.

### 2. The budget that binds was never the one being raised

B3 raised `turn_limit` 8 → 12 and `max_actions_per_session` 5 → 8. Session 5 found
`max_output_tokens` was absent from `minds_eye_preferences` entirely and raised it too. Session 6
then found that **none of the three was the active constraint**: the loop runs all its turns
inside ONE 240s Lambda invocation, so a round dies at turn 3–4 and `turn_limit: 12` is
unreachable — silently, with no notification, no session write and no retry. AC6 moved from
PARTIAL to **NOT MET** on that discovery, which is the honest direction for a status to move.

**Carry-forward: raising a limit is not the same as finding the limit.** Three numbers were
visible and adjustable; the binding one was structural and had to be measured. Before raising any
budget, establish which one is actually being hit.

### 3. A check can be missing its reach rather than its rule

D4 was scoped as *"L1 should flag `{{key.N}}` numeric indexing on a value that is not a known
array"* — a correct rule, and it would have shipped **without catching its own specimen**. The
first test failed because L1's template walk took `Object.values(s.input)`, top-level strings
only, so the token in `input.filters[0].value` was invisible to the walk entirely. The
unresolved-key check could not see it either. **That, not the missing numeric rule, is why D1
reached production through a clean L1.**

**Carry-forward: when a validator "should have caught X", ask first whether it could see X.** A
missing rule and an unreachable input produce the same clean pass, and only one of them is
visible in the rule list.

### 4. Deferring for measurement validity is not deferring for time

AC8 and A5 were both deferred to Sprint 10 — not because the sprint ran out of room, but because
running them now could not answer them. Session 1122 was already an AC8-shaped session: $3.40, no
completed repair, two failures transcript-driven rather than reasoning-driven. A round that dies
from a 54k-token transcript measures the harness, not Novia's Generation-domain scope, and a
*failed* AC8 under those conditions would enter the record as a capability finding when it is a
harness finding — contaminating the exact evidence the §12.7 dissolution decision is gated on.

The user's correction on A5 made the rule general: A5 was pulled into the sprint on "data carries
no deploy risk", which applied the AC8 reasoning inconsistently, since A5 needs a live round to
validate too. **Carry-forward: a test whose failure mode is indistinguishable from a known open
defect is not yet a test. Fix the defect, then run the test.**

### 5. The cost answer does not favour the new path, and saying so is the point

AC9 is the evidence the dissolution decision is gated on, and it came back **$2.73** to build
against a **$1.42** baseline, plus **$3.40** for a repair session that did not finish. The
temptation was to book the AC as met — it was measured, which is what it asked for — and read the
number generously. It is recorded as *measured and unfavourable*, with the three qualifications
that actually bear on it (harness defects since fixed, a diagnosed ~12× structural cut still
unbuilt, and a baseline that is not like-for-like since `create_workflow` delivered 4 surviving
workflows from 98 runs). §12.7 OQ1 now says the decision **should not be taken on this number as
it stands**.

### 6. Sprint 8's watch-list item held — once deliberately

Sprint 8's retro named *a rule true of its specimen, promoted to an unconditional* as the
signature failure, four times over. This sprint it was actively resisted: `ordered` was **not**
carried onto the `human_gate` contract because nothing in the rendering rule reads it; the
`option_source` value is read off the step rather than declared, with the one shape that misreads
named alongside it; D4's numeric check states its exception (`js_transform` and `llm_call` stay
unknown) in the code comment, not just the commit. The bounded-drift item — the fourth row of
that table — was **dropped**, not relaxed, once §12.11 showed the drift was a dialog strategy
expanding an interaction point.

### 7. Every defect this sprint was found by doing the work, not by reviewing it

Six harness defects in session 6, three in session 5, two contract faults from pointing L0 at
existing seeds, and D4's reach gap from writing one test. None came from a review pass. The
sprint's premise was that a real build would surface what inspection could not, and the ledger is
one-sided enough to state plainly: **the build is the audit.**
