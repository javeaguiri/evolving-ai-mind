...
| Item | Priority | Notes |
|---|---|---|
| Workflow safety guards (velocity detector, execution accumulator, cycle detector) | High | Required before Step Processor is production-ready. Right-brain can monitor `PGC_WorkflowStats` for anomalous run patterns and flag suspect workflows proactively |
...
| `v3.2-architecture-semantic-validation` | Semantic validation rules documented. Tech debt entry added |
...
| | — velocity detector, execution accumulator, cycle detector | ⬜ deferred — see tech debt register |
