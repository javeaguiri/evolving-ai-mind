...
#### 4.3.2 PGC Workflow Tables — Current State (Bootstrapped)
...
##### PGC_WorkflowRun
One row per workflow execution. Holds the execution stack, accumulated state, and callback routing.
...
| stack | jsonb | Execution stack — array of FrameDefinition (see Section 10). Controls frame flow only |
| state | jsonb | ✦ Accumulated cross-step data bag. Steps write `output_key` values here; subsequent steps read from here. Separate from `stack` which controls flow |
...
| retry_count | integer | ✦ How many attempts were made before this final status. Default 0 |
...
##### PGC_WorkflowStats (SQL view — not a table)

Computed on read over `PGC_WorkflowRun`. No writes required.
Queried by PROC when building prompts for workflow evaluation or improvement.
Registered in `PGC_TableMap.views` on the `PGC_WorkflowRun` row.

| Column | Type | Source |
|---|---|---|
| workflow_id | integer | `PGC_WorkflowRun.workflow_id` |
| run_count | integer | `COUNT(*)` |
| failure_count | integer | `COUNT(*) WHERE status = 'failed'` |
| success_count | integer | `COUNT(*) WHERE status = 'completed'` |
| last_run_at | timestamptz | `MAX(created_at)` |
| last_status | text | Status of the most recent run |
...
| input_variables | jsonb | ✦ Variables this prompt expects — `[{ name, description, required }]`. Documents the contract for prompt improvement |
...
None of them affect workflow execution — they are read-only from the execution engine's perspective.

##### PGC_SystemContext
Compact runtime self-description of the system. Injected into every heavy-lift LLM prompt
so the model knows what it is operating inside, what it can do, and what the rules are.
Replaces `architecture.md` at runtime. Seeded on bootstrap.

| Column | Type | Notes |
|---|---|---|
| id | serial PK | |
| key | text UNIQUE | Logical name — e.g. `system_overview`, `naming_conventions`, `allowed_column_types`, `serv_endpoints`, `sqs_message_format` |
...
| # | Table | Status |
|---|---|---|
| 1 | PGC_Schema | existing + 1 new column (`domain`) |
| 2 | PGC_TableMap | existing + 1 new column (`domain`) |
| 3 | PGC_EntitySchema | unchanged |
| 4 | PGC_DomainHelp | unchanged |
| 5 | PGC_Workflow | existing + 4 new columns (`domain`, `run_count`, `failure_count`, `last_run_at`) |
| 6 | PGC_WorkflowRun | existing + 2 new columns (`trace_id`, `triggered_by`) |
| 7 | PGC_WorkflowRunStep | unchanged |
| 8 | PGC_Prompt | existing + 4 new columns (`input_variables`, `output_schema`, `output_sample`, `error_log`) |
