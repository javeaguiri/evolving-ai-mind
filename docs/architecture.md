...
Version: 3.2  
Status: Active development — Intent Preprocessor redesign complete, implementation next  
Last updated: 2026-04-02 (session 17 — intent preprocessor two-pass redesign + pgvector integration)
...
PASS 1 — Intent Map Match (zero LLM)
  ...
  │     confidence: 'exact'
  │     e.g. "add my recipes carbonara" → add_recipes workflow
  │     e.g. "build me a domain"        → heavy_lift → CREATE_DOMAIN
  │
  ├── Match found, action_type = 'crud', field=value pairs present
  │     Extract domain from intent_category
  │     Build ad_hoc_step via matchCrudVerb()
  │     SHORT-CIRCUIT → executeCrudStep() (zero LLM, zero WorkflowRun)
  │     confidence: 'exact'
  │     e.g. "add my recipes name=Pasta" → direct serv_insert
  │
  ├── Match found, action_type = 'crud', no field=value pairs
  │     Extract domain from intent_category
  │     CHECK pre-loaded PGC_Workflow rows for this domain
  │       Workflow exists → route to it (e.g. add_recipes for ambiguous add)
  │       No workflow      → Tier 2 with domain hint
  │
  └── No match → PASS 2
  │
  ▼
PASS 2 — Domain-Workflow Lookup (zero LLM)
  ...
  │       Not found → Tier 2 (no hint)
  │
  └── Domain resolved → WORKFLOW KEYWORD SCAN
        Filter pre-loaded PGC_Workflow rows by domain
        Test lowercased input for token presence against each workflow's intent_keywords array
        Disambiguation: get_<domain> wins over list_<domain> when input contains terms
          beyond the verb and domain name (indicating a search term)
        Extract search_term when matched workflow is a retrieval type
        │
        ├── Keyword match found → route to matched workflow
        │     confidence: 'keyword_match'
        │     search_term populated if retrieval workflow
        │     e.g. "get my sweet potato recipe" → get_recipes, search_term: "sweet potato"
        │     e.g. "show all my recipes"        → list_recipes
        │     e.g. "add pasta to my recipes"    → add_recipes
        │
        ├── No keyword match, field=value pairs present
        │     Domain has no registered workflow for this verb
        │     Build ad_hoc_step (CRUD path) → executeCrudStep()
        │     confidence: 'crud'
        │     e.g. "recipes name=Pasta calories=400" → serv_insert
        │
        └── No keyword match, no field=value pairs → Tier 2 with domain hint
  │
  ▼ (no Pass 1 or Pass 2 match)
...
|---|---|---|---|
| `crud` | Pass 1 match → `executeCrudStep()` direct | Root table only — single-row INSERT / SELECT / UPDATE / DELETE | Zero LLM, zero WorkflowRun |
| `workflow` | Pass 1 or Pass 2 match → `handoff()` → Step Processor | Full domain entity — root + child tables, validation gates, multi-step | WorkflowRun lifecycle |

The `create_domain` workflow registers four intent rows per domain in `PGC_IntentMap`.
All four default to `action_type: workflow` — routed to the Step Processor.
This allows `add_<domain>` to invoke the LLM-parse-first workflow for natural-language
input, while update and delete use their confirmation-gate workflows.

**Pass 2 CRUD fallback rule:** Pass 2 reaches the `ad_hoc_step` CRUD path only when
the domain is resolved but no `intent_keywords` match is found AND the input contains
explicit `field=value` pairs. Without them, it falls through to Tier 2. This prevents
Pass 2 from intercepting natural-language add requests meant for the Step Processor.
...
Pass 2's CRUD fallback handles structured `field=value` insert input as a zero-cost
ad_hoc CRUD step when the user explicitly provides field values. Without them, Pass 2
falls through to Tier 2 — the workflow path is the default for natural-language add requests.
