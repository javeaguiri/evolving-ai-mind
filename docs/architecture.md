...
| run_count | integer | ✦ Incremented by PROC on each WorkflowRun create. Denormalised for prompt injection |
| failure_count | integer | ✦ Incremented by PROC when WorkflowRun status → `failed`. Denormalised for prompt injection |
| last_run_at | timestamptz | ✦ Updated by PROC on each WorkflowRun create |
| version | integer | |
