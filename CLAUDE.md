...
1. **LLM replay harness** — LIVE as of 2026-07-16, see `docs/sprints/CURRENT.md` and `docs/arch-replay.md`. `llm-harness` serves a recorded response from `PGC_SessionEntry` instead of calling Perplexity, keyed by a **content fingerprint of the assembled request** — seven component hashes (prompt/input/user_input/model/schema/memory/system_context), computed at the `callLlm` seam. Gates stay real. No SERV/SQS stubbing. Prompt iteration still costs the suffix from the changed step onward.
   > **Do not regress to "keyed by run + step, gated on version match"** — that was the original sketch and it was rejected on 2026-07-13. A run+step key still *resolves* after a path change and returns a response generated against a different prompt: **a silently wrong hit is worse than a miss.** Proven in practice — `create_workflow` step 21 runs twice per path (design, then regeneration) and records twice under one `step_id`; only a content fingerprint distinguishes the passes. Version match fails the same way: a prompt can change without its version doing so, and `upsert-prompt` overwrites text **in place**, so old versions do not exist to match against.
