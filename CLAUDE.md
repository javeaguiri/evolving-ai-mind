...
**The sprint's real work was that loop.** Three backlog entries turned out to be **one mistake made three times — truncating by bytes where the correct operation is summarising by structure, and doing it silently.** The rule now stands in `architecture.md` §1.5: **a bounded view must carry its own provenance — how much was withheld, and how to get the rest.** Measured before designing, and the measurement inverted the plan: across 898 Novia tool entries, 50 (5.6%) exceeded the 15,000-character cap, worst discarding 87.6% of 120,821 characters — but by tool, `query_table` was **28** and `read_workflow`, the specimen the backlog was written from, only **5**. Shipped: `capOutput` naming what it withheld and the entry sequence holding it (the handle is `sequence_number`, never `id`, or the rebuild diverges from the in-round render and forfeits the round's prefix cache credit); `read_session_entry` as a bounded pager; `read_workflow` `outline`/`steps`; `query_table` `columns`, which `getRows` had accepted since Sprint 10 and nothing passed; `diffStepFields` replacing **both** copies of a six-field allow-list; an explicit limit on both `PGC_SessionEntry` loads; and `output_snapshot` as `{ summary, chars, structure }`. 997 → **1044** unit tests.

**Two defects were found that nobody had filed.** A session past **100 entries** would load only its oldest 100, derive `nextSeq` from that page, collide with `uq_pgc_sessionentry_session_seq` and die on every subsequent resume — latent since Novia shipped, masked because the 240s wall kills sessions at ~80 entries first. And the engine defect had an **instruction twin**: `minds_eye_system_prompt` v31 said *"copy every unaffected step exactly as returned by read_workflow"* and *"never call read_workflow for a workflow already in this session"*, which against a 19,125-character workflow and a 15,000-character cap instructs the impossible and then forbids the recovery. That is what *"I composed step 13's message_template from memory"* actually was. Fixed as v32; `minds_eye_context_index` v3 dates the `output_snapshot` change rather than flipping it, because both shapes are in the table.

**Validated live, session 1177.** A cold session, a symptom-only prompt written as a household owner would write it. Her first action on the workflow was `read_workflow { outline: true }` — unprompted. Four targeted reads, largest 13,495 characters against a 15,000 cap, **zero truncation events**, ~22,000 characters of step content delivered whole. The gate then surfaced `options` and `message_template` changes that the old allow-list would have hidden — the v5→v6 failure reproduced on the very next repair, and caught. Approved as `process_receipt` v8.

Read `docs/sprints/sprint-11.md` (outcome, validation, retro) before scoping Sprint 12.

### Open Work (carry-forward to Sprint 12)

1. **`propose_workflow_fix` should accept a patch of complete steps — Sprint 12's opener, before any new workflow is built.** The tool takes the COMPLETE array, so repairing one step means resubmitting all of them; that is the root of the whole 2026-08-27 defect class. **Granularity is the step, not the field** (decided 2026-08-30): a patch is a set of whole steps merged by `step` identifier, so every submitted step stays a valid simulatable unit and the gate diff is exact rather than inferred. See backlog High Priority.
...
3. **The vector thresholds, assigned to Novia** (Sprint 11 AC3, not started). Two to calibrate, not one: step 8 is English→English on `name_embedding`, step 8c is raw-string→raw-string on `alias_name_embedding` — both still at the inherited **0.4**, which came from a cross-lingual comparison that exists on neither step. Memory 298's bands were measured against neither pairing. Three wrong merges across runs 780/782 are the specimens.
...
5. **No route to recategorise or aggregate an inventory item's category.** Reported as a dialog problem; it is one of the correction workflow's four verbs, so it rides with item 2. The reveal-scrolling half of this item was fixed in Sprint 11 (AC4).
...
11. **Release-readiness — scoped into Sprint 12 (AC6 decision, 2026-08-30).** Test environment, README bootstrap, log hygiene. Five deferrals ended. The reason it goes first: Sprint 12 builds a workflow and calibrates thresholds against live data, and the interim *deploy-to-prod-then-validate* process is what a test environment removes.
