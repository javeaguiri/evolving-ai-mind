...
const STEP_TYPES = [
  ...
  },
  {
    ...
    description: 'Two modes — mutually exclusive. (1) transform_type: named built-in transform. Built-ins: columnSummary, buildHelpOptions, resolveHelpContent, formatRecordList, buildChildInserts. (2) expression: pure synchronous JS expression evaluated via acorn AST gate + vm.runInNewContext(200ms). expression binds the resolved input_key value as "items". No async, no I/O, no Node globals. Use for score tallies, filters, aggregations, string formatting. Example: items.filter(r => r.score > 0).length',
    input_contract: [
      { field: 'transform_type', type: 'string', required: false, description: 'Named built-in — one of: columnSummary, buildHelpOptions, resolveHelpContent, formatRecordList, buildChildInserts. Mutually exclusive with expression.' },
      { field: 'expression',     type: 'string', required: false, description: 'JS value expression. input_key value bound as "items". No return, no semicolons. Example: "items.reduce((a,r) => a + r.score, 0)". Mutually exclusive with transform_type.' },
      { field: 'input_key',      type: 'string', required: false, description: 'Dot-path into local_state resolving to the input value bound as "items". Required when using expression or built-ins that read a single input.' },
      { field: 'output_key',     type: 'string', required: true,  description: 'Dot-path where the result is written to local_state' },
      { field: 'on_success',     type: 'string', required: false, description: 'Routing token — supports step:N for backward loops' },
    ],
    output_contract: [
      { field: 'output_key', type: 'any', description: 'Result of the expression or built-in transform written to local_state[output_key]' },
    ],
    on_success_options: STANDARD_ON_SUCCESS,
    on_failure_options: STANDARD_ON_FAILURE,
    status: 'live',
  ...
  },
  {
    ...
    input_contract: [
      { field: 'input.entityName', type: 'string', required: true,  description: 'Entity name matching PGC_EntitySchema.entity_name — e.g. "Recipe"' },
      { field: 'input.filters',    type: 'array',  required: false, description: 'Array of { column, op, value } filter objects applied to the root table' },
      { field: 'input.limit',      type: 'integer',required: false, description: 'Maximum number of entities to return' },
      { field: 'output_key',       type: 'string', required: true,  description: 'local_state key where the entities array is written' },
      { field: 'on_success',       type: 'string', required: false, description: 'Routing token' },
      { field: 'on_failure',       type: 'string', required: false, description: 'Routing token' },
    ],
    output_contract: [
      { field: 'output_key', type: 'array', description: 'Array of assembled entity objects — root columns plus child arrays' },
    ],
    on_success_options: STANDARD_ON_SUCCESS,
    on_failure_options: STANDARD_ON_FAILURE,
    status: 'live',
  },
  {
    ...
    input_contract: [
      { field: 'input.entityName', type: 'string',  required: true,  description: 'Entity name matching PGC_EntitySchema.entity_name — e.g. "Recipe"' },
      { field: 'input.id',         type: 'integer', required: true,  description: 'Primary key id of the root entity row' },
      { field: 'output_key',       type: 'string',  required: true,  description: 'local_state key where the assembled entity object is written' },
      { field: 'on_success',       type: 'string',  required: false, description: 'Routing token' },
      { field: 'on_failure',       type: 'string',  required: false, description: 'Routing token' },
    ],
    output_contract: [
      { field: 'output_key', type: 'object', description: 'Assembled entity object — root columns plus child arrays. Single object, not an array.' },
    ],
    on_success_options: STANDARD_ON_SUCCESS,
    on_failure_options: STANDARD_ON_FAILURE,
    status: 'live',
  },
  {
    ...
    input_contract: [
      { field: 'input.entityName', type: 'string', required: true,  description: 'Entity name matching PGC_EntitySchema.entity_name — e.g. "Recipe". Supports {{template}} substitution.' },
      { field: 'output_key',       type: 'string', required: true,  description: 'local_state key where the full schema object is written' },
      { field: 'on_success',       type: 'string', required: false, description: 'Routing token' },
      { field: 'on_failure',       type: 'string', required: false, description: 'Routing token' },
    ],
    output_contract: [
      { field: 'output_key', type: 'object', description: '{ entity_name, description, root: { table, columns: [{name,type}] }, children: [{ table, alias, fk_column, output_key, columns }] }' },
    ],
    on_success_options: STANDARD_ON_SUCCESS,
    on_failure_options: STANDARD_ON_FAILURE,
    status: 'live',
